...
class Relation
{
    ...
    int dup;
    int cost = 0; //
    string tab_file_path;   //path of that table as like c:Desktop\ff\ etd
    ifstream tab_file;      // object of needed to read file content using some funtion that are present in ifsteam library

    string table_name;   // like first table will be named as R ans second one as S
    string col_info;   // name of all column
    string table_nam; // name as it present on file(like customer , product_info etc)
...
public:
    ...
    void create_sorted_run()
    {
        ...
        while (getline(tab_file, line))
        {
            if (tab_file.good() && !line.empty())
            {
                ...
                if (file_vec.size() == MB * TB)
                {
                    ...
                    //path where , our sorted runs will be saved 
                    string pat = path_to_store + "\\" + table_name + "\\" + "_" + to_string(idx) + ".txt";
                    sorted_runs_path.push_back(pat);
                    //writing the content from memory to disc
                    wri_buf(file_vec, pat);
                    //increamenting the sorted runs count
                    idx++;
        ...
        //calculating total numner of record or rupble in table
        n = (idx - 1) * TB * MB + file_vec.size();
        file_vec.clear();
        //calculating the total number of block 
    ...
    {  
        //creating an object of ofstream class to write this content of that file
        ofstream tem_file(path);
        for (auto i : vec)
        {
            tem_file << i.second << "\n";
        }
        //closing this file that open to write 
        tem_file.close();
    }
...
int main(int argc, char *argv[])
{
    ...
    S_path = argv[2];
    // third input  memory frames  available
    int M = stoi(argv[3]);
    ...
    cout << "Total cost of Externsl-Sort-Merge-Join(Algorithm) : " << ALL_cost << "\n";
    cout << "\n\n\n=================================================================================================================================================\n\n";
    return 0;
}