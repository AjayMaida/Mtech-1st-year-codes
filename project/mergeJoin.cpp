...
class Relation
{
    ...
    int b;       // total number of clock in relation
...
public:
    ...
    bool full_container(int sorted_file_num, vvvs &container_queue, vector<ifstream *> const &sorted_files, int B)
    {
        string line;
        int i = 1;
        bool flag = false;
        while (getline(*sorted_files[sorted_file_num], line) && i <= TB * B)
        {
            ...
            container_queue[sorted_file_num].push_back({key, to_string(sorted_file_num), line});
            i++;
        }
        cost += ceil((float)(i - 1) / (float)TB);
        return flag;
    }
    string merge_sorted_runs()
    {
        ...
        vvvs container_queue(sorted_runs_path.size());
        priority_queue<vs, vvs, cmp> final_sorted_que;
        ...
        while (i < sorted_runs_path.size())
        {
            ...
            while (getline(*sorted_runs_file[i], line) && j <= block_to_read_from_sorted_run * TB)
            {
                ...
                container_queue[i].push_back({key, to_string(i), line});
                // cout << key << "\n";
                j++;
            }
        ...
        cost += block_to_read_from_sorted_run * sorted_runs_path.size();
...
int Cost(){return cost;}
    ...
    MergeJoin(string path1, string path2, int m)
    {
        MB = m;
        block_of_each_rel = (MB - 1) / 2;
...
{
    // cout << "\tphaseOne of External sort Merge\n";
    table->create_sorted_run();
    // cout << "\tphaseTwo of External sort Merge\n";
    table->dis();
    return table->merge_sorted_runs();
}
int main(int argc, char *argv[])
{
    ...
    R_path = argv[1];
    S_path = argv[2];
    int M = stoi(argv[3]); // memory block available
    ...
    cout << "";
    cout << "\tRelation R sorting started :\n";
    final_r = Sort_the_table(R_table);
    cout << "Relation R's sorted file path : " << final_r << "\n\n\n";
    cout << "\tRelation S sorting started :\n";
    final_s = Sort_the_table(S_table);
    cout << "Relation S's sorted file path : " << final_r << "\n\n ";
    cout << "Total cost of sorting two Relation is : " << ALL_cost << "\n\n";
    cout << "Joining the tables : \n";
    MergeJoin(final_r, final_s, M);
